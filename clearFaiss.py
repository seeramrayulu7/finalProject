...
def clear_vector_store(text_chunks):   
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    index_path = "faiss_index"
    vector_store = FAISS.load_local(
            index_path, embeddings, allow_dangerous_deserialization=True
...
def skinDiseasePrediction(image):
    ...
    if (prediction>0.5):
        user_input("The user has uploaded an image just now and there is High chance of presence of "+arr_of_dis[index]+" !! Need to Consult a Doctor immediately!! Please respond in the context to this disease until another image is uploaded. Also Please give the disease that is classified in bold letters first and then tell the user about it.",True)
    else:
