...
def user_input(user_question, flag):
    ...
    if st.session_state.logged_in:
        save_user_interaction(st.session_state.username, 
                            user_question, 
...
def add_user(username, password):
    ...
    try:
        ...
        c.execute("INSERT INTO users VALUES (?, ?, ?, ?)", 
                 (username, hashed_pw, datetime.now(), datetime.now()))
        conn.commit()
        return True
...
def save_user_interaction(username, question, answer):
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    ...
    c.execute("INSERT INTO user_history VALUES (?, ?, ?, ?)",
             (username, question, answer, datetime.now()))
    conn.commit()
    conn.close()
# Main function
def main():
    ...
    if not st.session_state.logged_in:
        st.title("Welcome to Health Chatbot")
